		...
		)
	{
			...
			if(!outputDirectlyBool_Phase1Only)
			{
				if(pairExistsBool) // some pair exists
				{
					...
					if(allAlignmentCompleteBool)
					{
						//cout << "start to check alignPairScoreTooLow_bool " << endl;

						//int alignment_score_min_output 
						//	= readInfo.returnAlignmentScoreMinOutput_withComplement(
						//		ALIGNMENT_SCORE_MIN_OUTPUT_COMPLEMENT);
						// int alignment_score_min_output 
						// 	= readInfo.returnAlignmentScoreMinOutput_withComplement_perHundredBases(
						// 		ALIGNMENT_SCORE_MIN_OUTPUT_COMPLEMENT_PerHundredBases);						
						// bool completeAlignmentPairScore_tooLow
						// 	= this->alignPairScoreTooLow_bool(alignment_score_min_output);
						bool completeAlignmentPairScore_tooLow = this->alignPairScoreTooLow_bool(readInfo);
						//bool completeAlignmentPairScore_tooLow = this->alignPairScoreTooLow_bool(ALIGNMENT_SCORE_MIN_OUTPUT);						
						...
						{
							resultArray->insert_PeAlignSamStrVec_complete(
					...
					else
					{
						if(!Do_Phase1_Only)
						{
							resultArray->insert_PeAlignInfoStrVec_inCompletePair(
								this->getTmpAlignInfoForFinalPair(
						 		readInfo.returnReadName_1(), readInfo.returnReadName_2(), 
								readInfo.returnReadSeq_1(), readInfo.returnReadSeq_2(),
								readInfo.returnReadQual_1(), readInfo.returnReadQual_2(),
								FastaOrFastq), tmpOpenMP);
						}
				...
				else // no pair exists: 1.one end unmapped; 2. both ends unmapped
				{
					...
					{	
						if(Do_Phase1_Only)
						{
							resultArray->insert_PeAlignInfoStrVec_oneEndUnmapped(
								this->getSAMformatForUnpairedAlignments_secondaryOrNot(
									readInfo, FastaOrFastq), tmpOpenMP);
						}
						else
						{
							resultArray->insert_PeAlignInfoStrVec_oneEndUnmapped(
					...
					{
						resultArray->insert_PeAlignSamStrVec_bothEndsUnmapped_mappedToRepeatRegion( 
							this->getSAMformatForBothEndsUnmapped_mappedToRepeatRegionReads(
								readInfo, repeatRegionInfoVec[threadNO], indexInfo,
								FastaOrFastq), tmpOpenMP);
					}
					...
					{
						resultArray->insert_PeAlignSamStrVec_bothEndsUnmapped(
							this->getSAMformatForBothEndsUnmapped(
								readInfo, FastaOrFastq), tmpOpenMP);						
					}
				}
			...
			{
				cout << "outputDirectlyBool_Phase1Only " << outputDirectlyBool_Phase1Only << endl;
				exit(1);
			}		
			//cout << "finish output_phase1 ..." << endl;
	}
